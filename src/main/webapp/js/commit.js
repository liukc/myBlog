...
editor.customConfig.customUploadImg = function (files, insert) {//对上传的图片进行处理，图片上传的方式
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            editor.cmd.do('insertHTML', '<img style="width: 590px;height: 350px;" src="'+ imgUrl+'" alt="">')
...
$("#btn1").click(function () {
    var centent = editor.txt.html();  // 获取 text格式的内容
        ...
        $.ajax({
            cache: true,//保留缓存数据
            type: "POST",
            url: "/blog/cn.forlkc.servlet/CommitServlet",//这是我在后台接受数据的文件名
            data: {
                "commit": JSON.stringify(arr)
            },
